...
Пространство ${L(\mathcal{H}_n)}$ наделено гильбертово-шмидтовым скалярным произведением:
...
\noindent которое естественно расширяет скалярное произведение в ${\mathcal{H}_n}$. Вещественное линейное пространство эрмитовых операторов далее обозначим как ${H(\mathcal{H}_n)}$.
...
Рассмотрим ряд примеров коммутационного и антикоммутационного соответствия в базисе Паули.
...
\begin{equation}\label{eq5}
    S_{KL} = i^{\omega_{KL}}(-1)^{\omega^-_{KL}},\;\;\;
    S_{(KL)} = \frac{S_{KL}}{2}\left(1 + (-1)^{\omega_{KL}}\right),\;\;\;
    S_{[KL]} = \frac{S_{KL}}{2}\left(1 - (-1)^{\omega_{KL}}\right),
\end{equation}

\noindent где ${S_{(KL)}}$ и ${S_{[KL]}}$ — симметричная и антисимметричная части ${S_{KL}}$. Значения ${S_{KL}}$, ${S_{(KL)}}$ и ${S_{[KL]}}$ приведены в таблице \ref{table2}.
